    ...
    public class ShippingCalculator : IShippingCalculator
    {
        public decimal CalculateShipping(Basket basket)
        {
            foreach (var lineItem in basket.LineItems)
            {
                ...
                if (basket.LineItems.FirstOrDefault(x => x.Shipping.GetType() == typeof(Shipping.ConditionalPerRegionShipping) && x.SupplierId == lineItem.SupplierId && x.DeliveryRegion == lineItem.DeliveryRegion && x != lineItem) != null)
                {
